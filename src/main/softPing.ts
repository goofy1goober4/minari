...
// Demo aid — MINARI_PING_FAST=1 cuts the dev soft-ping min-spacing to 30s so
// pings land quickly on camera. tick / fire_prob / daily cap are untouched;
// the override reverts when the env var is unset, and production is
// unaffected.
const SUPPRESSION_CONFIG: SuppressionConfig = !IS_DEV
  ? PROD_SUPPRESSION_CONFIG
  : process.env.MINARI_PING_FAST === '1'
    ? { ...DEV_SUPPRESSION_CONFIG, minSpacingMs: 30 * 1000 }
    : DEV_SUPPRESSION_CONFIG;
